...
export default function Home() {
  //set up all needed states //
  // keep track if the user's wallet is connected
  const [walletConnected, setWalletConnected] = useState(false);
  // keep track if the current wallet has been whitelisted or not
  const [joinedWhitelist, setJoinedWhitelist] = useState(false);
  // loading is set to true when we are waiting for a transaction to be minded
  ...
  // create all needed functions below //
  /**
   ...
   *
   * A `Signer` is a special type of Provider used in case a `write` transaction needs to be made to the blockchain, which involves the connected account needing to make a digital signature to authorize the transaction being sent. 
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 5) {
      window.alert('Change the network to Goerli Test Network');
      throw new Error('Change network to Goerli Test Network');
    }
    if (needSigner) {
  ...
  const getNumberOfWhitelisted = async () => {
    try {
      ...
      const _numberOfWhitelisted =
        await whitelistContract.numberOfWhitelisted();
